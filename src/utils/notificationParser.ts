...
export function parseNotificationText(
  ...
  // Strategy B: "at [Vendor]" (e.g., "charged $14.50 at Starbucks", "Purchase of $25.00 at Trader Joe's was approved")
  if (!rawVendor) {
    const atMatch = trimmed.match(/\bat\s+([A-Za-z0-9\s'&.*#\-]+?)(?:\s+(?:was|is|on|with|for|using|via|card|ending|has|have|dated|\d{4})\b|\.|\,|$|\n)/i);
  ...
  if (!rawVendor) {
    const toMatch = trimmed.match(/(?:paid|sent|payment|transfer(?:red)?)\s+(?:(?:to)\s+)?(?:(?:\$|\w+)?\s*\d+(?:\.\d{2})?\s*(?:to\s+)?)?([A-Za-z0-9\s'&.*#\-]+?)(?:\s+(?:was|is|with|using|on|via|from|for|has|card|ending)\b|\.|\,|$|\n)/i);
  ...
  if (!rawVendor) {
    const fromMatch = trimmed.match(/(?:charge|transaction|purchase)\s+from\s+([A-Za-z0-9\s'&.*#\-]+?)(?:\s+(?:was|is|for|on|with|via|has|card|ending)\b|\.|\,|$|\n)/i);
